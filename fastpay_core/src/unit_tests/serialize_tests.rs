...
fn test_order() {
    ...
    let request_order = RequestOrder::new(request.into(), &sender_key);
    ...
    let request_order2 = RequestOrder::new(request2.into(), &sender_key);
...
fn test_time_order() {
    ...
    let now = Instant::now();
    for _ in 0..100 {
        let request_order = RequestOrder::new(request.clone().into(), &sender_key);
