...
fn test_rotate_key_pair() {
    ...
    rt.block_on(sender.transfer_to_fastpay(Amount::from(3), dbg_account(2), UserData::default()))
        .unwrap();
}
...
fn create_and_transfer_coins(coins: Vec<TransparentCoin>) -> Result<(), failure::Error> {
    ...
    let assets = rt.block_on(client1.spend_and_create_coins(
        Amount::from(3),
        Vec::new(),
        coins.clone(),
        Vec::new(),
    ...
    for coin in coins {
        let certificate = rt.block_on(client2.spend_and_transfer(
            coin.seed,
            Address::FastPay(dbg_account(3)),
            UserData::default(),
        ))?;
        assert!(matches!(&certificate.value, Value::Confirm(
            Request {
                operation: Operation::SpendAndTransfer { recipient: Address::FastPay(id), .. },
                ..
            }) if id == &dbg_account(3)
        ));
    ...
    assert!(rt
        .block_on(client2.spend_and_transfer(
            3,
            Address::FastPay(dbg_account(3)),
            UserData::default()
        ))
...
fn test_open_account() {
    ...
    assert!(rt
        .block_on(sender.transfer_to_fastpay(Amount::from(3), new_id.clone(), UserData::default()))
        .is_ok());
    ...
    assert!(rt
        .block_on(client.transfer_to_fastpay(Amount::from(3), dbg_account(3), UserData::default()))
        .is_ok());
}
