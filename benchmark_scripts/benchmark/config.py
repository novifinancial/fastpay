...
class Key:
    ...
    def from_file(cls, filename):
        ...
        with open(filename, 'r') as f:
            data = load(f)
        return cls(data['authority']['name'], data['key'])
...
class Committee:
    ...
    """
    def __init__(self, addresses, network_protocol="TCP"):
        ...
        for name, shards in addresses.items():
            self.json['authorities'] += [{
                'name': name,
    ...
    def __init__(self, filename):
        ''' Read a committee file generated by the rust codebase '''
        assert isinstance(filename, str)
        with open(filename, 'r') as f:
            data = load(f)
        ...
        for authority in data['authorities']:
            name = authority['name']
            network_protocol = authority['network_protocol']
            host = authority['host']
            base_port = authority['base_port']
            num_shards = authority['num_shards']

            shards = [
                f'{host}:{base_port+i}' for i in range(num_shards)
            ]
            self.json['authorities'] += [{
                'name': name,
                'network_protocol': network_protocol,
    ...
    def shards(self):
        ''' Returns the number of shards per authority. '''
        return len(self.json['authorities'][0]['shards'])
